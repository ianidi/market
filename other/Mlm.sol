...
contract ETHStvo is Ownable {
    ...
    event prolongateStarEvent(
        address indexed _user,
        uint256 _star,
        uint256 _time
    );
    event getMoneyForStarEvent(
        address indexed _user,
    ...
    mapping(uint256 => uint256) public STAR_PRICE;
    uint256 REFERRER_1_STAR_LIMIT = 3;
    uint256 PERIOD_LENGTH = 3650 days;

    struct UserStruct {
        bool isExist;
        uint256 id;
        uint256 referrerID;
        uint256 referrerIDInitial;
        address[] referral;
        mapping(uint256 => uint256) starExpired;
    }
    ...
    function regUser(uint256 _referrerID) public payable {
        ...
        uint256 _referrerIDInitial = _referrerID;

        if (
            users[userList[_referrerID]].referral.length >=
            REFERRER_1_STAR_LIMIT
        ) {
    ...
    function buyStar(uint256 _star, uint256 _cycle) public payable {
        ...
        if (_star == 1) {
            ...
            for (uint256 l = _star - 1; l > 0; l--) {
                require(
                    users[msg.sender].starExpired[l] >= now,
                    "Buy the previous star"
                );
            }
    ...
    function payForStar(
        ...
        if (users[referer].starExpired[_star] >= now) {
            ...
            if (_star >= 3) {
                if (_star == 5) {
                    bool result;
                    result = address(uint160(userList[1])).send(
                        uint256(2.25 ether)
                    );
                    money = SafeMath.sub(money, uint256(2.25 ether));
                }
                ...
                if (_star == 9) {
                    ...
                    money = SafeMath.sub(money, uint256(0.50 ether));
                }

                if (_star == 13) {
                    bool result;
                    result = address(uint160(userList[1])).send(
                        uint256(225.00 ether)
                    );
                    money = SafeMath.sub(money, uint256(225.00 ether));
                }
                ...
                if (_star == 17) {
                    ...
                    money = SafeMath.sub(money, uint256(5.00 ether));
                }

                if (_star == 21) {
                    bool result;
                    result = address(uint160(userList[1])).send(
                        uint256(2250.00 ether)
                    );
                    money = SafeMath.sub(money, uint256(2250.00 ether));
                }

                bool result_one;
                result_one = address(uint160(referer)).send(
                    SafeMath.div(money, 2)
                );

                bool result_two;
                result_two = address(uint160(refererInitial)).send(
                    SafeMath.div(money, 2)
                );
            } else {
                bool result;
